...
- Add a frozen `BankProfile` dataclass holding the only genuinely bank-specific values: detection substrings, date formats, skip prefixes, and sign convention.
- Ship exactly two profiles: `BOFA` (tested against real statements) and `GENERIC` (fallback). **No speculative profiles for untested banks.**
